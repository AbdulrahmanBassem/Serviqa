...
export const Inventory = () => {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm("Are you sure you want to delete this item?")) deleteItem(id);
  };
            ...
            <tbody>
              {items?.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <span style={{ 
                      color: item.quantity <= 5 ? "var(--color-danger)" : "inherit",
                  ...
                  <td style={{ textAlign: "right" }}>
                    <button onClick={() => handleOpenModal(item)} className={styles.actionBtn} title="Edit">
                      <Edit2 size={18} />
                    </button>
                    <button onClick={() => handleDelete(item.id)} disabled={isDeleting} className={styles.actionBtn} title="Delete">
                      <Trash2 size={18} />
                    </button>
                  </td>
                </tr>
